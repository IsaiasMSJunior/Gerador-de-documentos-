...
# --- Firebase via st.secrets["firebase"] ---
conf = st.secrets["firebase"]
service_account_info = json.loads(conf["serviceAccount"])
api_key              = conf["apiKey"]
...
if "extras" not in st.session_state:
    e=carregar_json("extras.json") or {}
    e.setdefault("metodologia",[]);e.setdefault("recursos",[]);e.setdefault("criterios",[])
    st.session_state.extras=e
if "pagina" not in st.session_state:     st.session_state.pagina="Cadastro de Professor"
if "professores" not in st.session_state: st.session_state.professores=carregar_json("professores.json") or []
if "turmas" not in st.session_state:      st.session_state.turmas=carregar_json("turmas.json")      or {}
if "horarios" not in st.session_state:    st.session_state.horarios=carregar_json("horarios.json")    or []

# --- Sidebar e navegação ---
pages=["Cadastro de Professor","Cadastro de Turmas","Cadastro de Horário",
       "Gerar Agenda e Plano","Cadastro Extras","Gerar Guia","Gerar Planejamento Bimestral"]
for p in pages:
...
if st.session_state.pagina=="Cadastro de Professor":
    ...
    disciplinas=st.multiselect("Disciplina(s)",["Arte","Ciências","Ed. Física","Ed. Financeira","Geografia","História","Português","Inglês","Matemática","PV","Redação","Tecnologia","OE Port","OE Mat"])
...
# 2. Cadastro de Turmas
    ...
    turma_map={"6º":["6ºA","6ºB","6ºC","6ºD"],"7º":["7ºA","7ºB","7ºC"],"8º":["8ºA","8ºB","8ºC","8ºD"],"9º":["9ºA","9ºB","9ºC","9ºD"],"1º":["1ºA","1ºB","1ºC","1ºD","1ºE"],"2º":["2ºA ADM","2ºB ADM","2ºC"],"3º":["3ºA","3ºA ADM","3ºB ADM","3ºB LOG"]}
...
# 3. Cadastro de Horário
    ...
    for i, itm in enumerate(st.session_state.horarios):
        ...
        if cols[5].button("X",key=f"rm_{i}"):
            st.session_state.horarios.pop(i); break
    if st.button("Salvar Horários"):
        salvar_json("horarios.json",st.session_state.horarios); st.success("Horários salvos!")
        ...
        for tab, turma in zip(tabs, turma_idx.keys()):
            with tab:
                ...
                for idx in turma_idx[turma]:
                    ...
                    opts=sorted(pd.Series(df_bank.loc[(df_bank["DISCIPLINA"]==h["disciplina"])&(df_bank["ANO/SÉRIE"]==extrai_serie(turma))&(df_bank["BIMESTRE"]==bim),"Nº da aula"].dropna().astype(int)).unique().tolist())
                ...
                if st.button("Gerar Plano",key=f"gera_plano_{turma}"):
                    arq=gerar_plano_template([e for e in entries if e["turma"]==turma],df_bank,prof,sem_sel,bim,turma,met_sel,rec_sel,crit_sel)
...
# 5. Cadastro Extras
    ...
    with tab3:
        ...
        for i,item in enumerate(st.session_state.extras["criterios"]):
            c1,c2=st.columns([0.9,0.1]);c1.write(f"- {item}");c2.button("X",key=f"del_crit_{i}",on_click=lambda i=i:st.session_state.extras["criterios"].pop(i) or salvar_json("extras.json",st.session_state.extras))
    ...
    if not st.session_state.horarios:
        ...
        for tab,turma in zip(tabs,turmas):
            with tab:
                ...
                met_sel=st.multiselect("Metodologia",st.session_state.extras["metodologia"],key=f"met_g_{turma}")
            ...
            with tab:
                ...
                for i in range(st.session_state[cnt_key]):
                    with st.expander(f"Planejamento {i+1}",expanded=True):
                        ...
                        semanas=[f"{w[0].strftime('%d/%m')} – {w[4].strftime('%d/%m')}" for w in calendar.Calendar().monthdatescalendar(ano_planej,meses.index(mes)+1) if all(d.month==meses.index(mes)+1 for d in w[:5])]
